...
def load_hyperparameters(config_file):
    '''
        ...
        Args: 
            config_file : the path of the json file
                        that contains the hyperparameters
    '''
...
def create_dirs():
    '''
        ...
        to save the results and the checkpoints
    '''
...
def init_params(target):
    '''
        Initialize the target model, its hyperparameters and the datasets

        Args: 
            target : The name of the target model
                    Choose between 'MNIST', 'CIFAR10' and 'HighResolution'
    '''
    if target == 'MNIST':
        batch_size = 128
...
def train_target_model(target, target_model, epochs, train_dataloader, test_dataloader, dataset_size):
    '''
        ...
        based on the hyperparameters defined

        Args
            target : The name of the target model
            target_model : The object of the trained target model
            epochs : The number of epochs to train the model
            train_dataloader :  The dataloader of the training set
            test_dataloader :  The dataloader of the test set
    ...
    # calculate and print the accuracy on the pristine data
    n_correct = 0
...
def test_attack_performance(target, dataloader, mode, adv_GAN, target_model, batch_size, l_inf_bound, dataset_size):
    '''
        Test the performance of the AdvGAN on a test set

        Args
            target : The name of the target model
            dataloader :  The dataloader on which you want to perform the tests
            mode : String value used just for the printing of the results
            adv_GAN : The trained AdvGAN object
            target_model : The object of the trained target model
            batch_size : The batch size
            l_inf_bound : The maximum perturbation allowed for a single pixel
            dataset_size : The size of the dataset
    '''
    n_correct = 0
    ...
    # save the true data and labels 
    # along with the adversarial images and their predicted labels
    ...
    # print the accuracy on the adversarial examples
    print(target)
    print('Correctly Classified: ', n_correct.item())
...
# train the AdvGAN
# if you want to use just the latest checkpoint
...
# test the performance of the AdvGAN
